...
def setup_environment_params(yaml_file_path):
    ...
    if RUN_PARAM_ONCE:
        warnings.warn(
            "WARNING: Environment parameter setup ran more than once. Environment might break or not work properly."
        )
    else:
...
def build_robot_table(robot_pose_array):
    ...
    if not RUN_PARAM_ONCE:
        raise RuntimeError(
            "You need to run setup_environment_params with the yaml file path before building the device table."
        )
    ...
    for i in range(len(robot_pose_array) - environment_data["number_of_sensors"]):
        new_point = Point(
            ...
            robot_pose_array[i].position.z,
        )
        new_quaternion = Quaternion(
            robot_pose_array[i].orientation.x,
            robot_pose_array[i].orientation.y,
            robot_pose_array[i].orientation.z,
            robot_pose_array[i].orientation.w,
        )
...
def build_sensor_table(sensor_pose_array):
    ...
    for i in range(len(sensor_pose_array) + environment_data["number_of_robots"]):
        new_point = Point(
            ...
            sensor_pose_array[i].position.z,
        )
        new_quaternion = Quaternion(
            sensor_pose_array[i].orientation.x,
            sensor_pose_array[i].orientation.y,
            sensor_pose_array[i].orientation.z,
            sensor_pose_array[i].orientation.w,
        )
